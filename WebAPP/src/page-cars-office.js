...
export default class PageCarsOffice
{
	constructor(args) 
	{
        this.app = args.app;
		args.app.LoadHTML('./page-cars-office.html', args.app.Main, () => 
		{
			...
			carListBody.addEventListener('click', (pointerCoordinates) => 
			{
				...
				{
                    this.bewegung = JSON.parse(localStorage.getItem('rentObject'));
					let kraftfahrzeug_id = pointerCoordinates.target.parentElement.dataset.kraftfahrzeugId;
                    this.bewegung.kraftfahrzeug_id = kraftfahrzeug_id;
					...
					window.open('#transactiondetails?bid=' + this.bewegung.bewegung_id, '_self');
				}
			});
				...
				for (let kraftfahrzeug of response) 
				{
					...
					`<div class="card cards" style="width: 18rem;">
						...
						<div class="card-body" data-kraftfahrzeug-id="${kraftfahrzeug.kraftfahrzeug_id}">
							...
							<a class="btn btn-primary" id="aEinzelheiten_${iterator}">Wählen</a>
						</div>
					</div>
					`;
					iterator++;

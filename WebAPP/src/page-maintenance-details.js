...
export default class PageMaintenanceDetails 
{
	constructor(args) 
	{
		this.app = args.app;
		...
		args.app.LoadHTML('./page-maintenance-details.html', args.app.Main, () => 
		{
            // Werkstatt
            const inputWerkstatt = args.app.Main.querySelector('#inputWerkstatt');
            const inputDatum = args.app.Main.querySelector('#inputDatum');
            const inputUhrzeit = args.app.Main.querySelector('#inputUhrzeit');
            const inputEnde = args.app.Main.querySelector('#inputEnde');
            const inputKosten = args.app.Main.querySelector('#inputKosten');
            const inputCheckboxEnde = args.app.Main.querySelector('#inputCheckboxEnde');
            const inputCheckboxBezahlt = args.app.Main.querySelector('#inputCheckboxBezahlt');
			...
			if(args.wtid)
			{
				let wartungstermin_id = parseInt(args.wtid);
				this.wartungsterminLaden(wartungstermin_id);
			}
			else
			{
			...
			buttonKfzSpeichern.addEventListener('click', (e) => 
			{
                ...
                if(!this.wartungsTermin)
                {
                    this.wartungsTermin = {};
                }

                this.wartungsTermin.werkstatt = inputWerkstatt.value;
                this.wartungsTermin.datum = inputDatum.value;
                this.wartungsTermin.uhrzeit = inputUhrzeit.value;
                this.wartungsTermin.vorraussichtliches_ende = inputEnde.value;
                this.wartungsTermin.kosten = inputKosten.value;

                if(inputCheckboxEnde.checked)
                {
                    this.wartungsTermin.erledigt = true;
                }
                else
                {
                    this.wartungsTermin.erledigt = false;
                }

                if(inputCheckboxBezahlt.checked)
                {
                    this.wartungsTermin.bezahlt = true;
                }
                else
                {
                    this.wartungsTermin.bezahlt = false;
                }

                var kraftfahrzeug_id = localStorage.getItem("kraftfahrzeug_id");
                this.wartungsTermin.kraftfahrzeug_id = kraftfahrzeug_id;

                this.app.ApiWartungsterminSet(() =>
                {
                    if (inputMarke.value && inputModell.value) 
				{
					if(kraftfahrzeug_id)
					{
						this.kraftfahrzeug.kraftfahrzeug_id = parseInt(kraftfahrzeug_id);
					}
					else
					{
						this.kraftfahrzeug = {};
    ...
    wartungsterminLaden(wartungstermin_id)
    {
        if(wartungstermin_id != null)
        {
            this.app.ApiWartungsterminGet((response) =>
            {
                if(response != null)
                {
                    inputWerkstatt.value = response.werkstatt;
                    inputDatum.value = response.datum;
                    inputUhrzeit.value = response.uhrzeit;
                    inputEnde.value = response.vorraussichtliches_ende;
                    inputKosten.value = response.kosten;

                    if(response.erledigt)
                    {
                        inputCheckboxEnde.checked = true;
                    }
                    else
                    {
                        inputCheckboxEnde.checked = false;
                    }

                    if(response.bezahlt)
                    {
                        inputCheckboxBezahlt.checked = true;
                    }
                    else
                    {
                        inputCheckboxBezahlt.checked = false;
                    }

                    localStorage.setItem("kraftfahrzeug_id", response.kraftfahrzeug_id);
                    this.kraftfahrzeugDatenLaden(response.kraftfahrzeug_id);
                }
                else
                {
                    alert("Der Wartungstermin wurde nicht gefunden!");
                }
            }, (ex) =>
            {
                alert(ex);  
	...
	kraftfahrzeugDatenLaden(kraftfahrzeug_id) 
	{
